...
from download_utils import maybe_download, download_path, unzip_file
...
def download_mind(size="small", dest_path=None):
    ...
    Args:
        size (str): Dataset size. One of ["small", "large","demo"]
...
def read_clickhistory(path, filename):
    ...
    for i in range(len(lines)):
        """
                userid  imp_time                click(clicks)                                                   (imp)pos      neg
...
def get_train_input(session, train_file_path, npratio=4):
    """Generate train file.(to train_file_path)

    train file format:
    [1(pos)/0(neg)] ["train_"][userid] [pos/neg]
...
def get_valid_input(session, valid_file_path):
    """Generate validation file (to valid_file_path)
    
    valid file format:
    [1(pos)/0(neg)] ["valid_"][userid] [pos/neg]%[sess_id]
...
def get_user_history(train_history, valid_history, user_history_path):
    ...
    user history file format:
    ["train_"][userid] ,[clicks]
    ["valid_"][userid] ,[clicks]
